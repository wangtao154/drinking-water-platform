package com.platform.auth.interceptor;
...
/**
 * JWT 认证拦截器
 * 从 Authorization header 解析 JWT，设置 UserContext
 */
@Slf4j
@Component
...
public class JwtAuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!jwtUtil.validateToken(token)) {
            response.setStatus(401);
            response.setContentType("application/json;charset=UTF-8");
            response.getWriter().write("{\"code\":40103,\"message\":\"token无效或已过期\",\"data\":null,\"timestamp\":\"" + java.time.LocalDateTime.now() + "\"}");
            return false;
        }
        ...
        if (currentUser == null) {
            response.setStatus(401);
            response.setContentType("application/json;charset=UTF-8");
            response.getWriter().write("{\"code\":40103,\"message\":\"token解析失败\",\"data\":null,\"timestamp\":\"" + java.time.LocalDateTime.now() + "\"}");
            return false;
        }
